...
from transformers.models.camembert.modeling_camembert import CamembertOutput, CamembertSelfAttention, CamembertSelfOutput
...
# Copied from transformers.models.bert.modeling_bert.BertSelfAttention with Bert->Camembert
class CamembertSelfAttentionWithAdapters(BertSelfAttentionAdaptersMixin, CamembertSelfAttention):
    def forward(
        self,
...
class CamembertSelfOutputWithAdapters(BertSelfOutputAdaptersMixin, CamembertSelfOutput):
...
class CamembertOutputWithAdapters(BertOutputAdaptersMixin, CamembertOutput):
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.bottleneck_layer_forward(hidden_states, input_tensor, self.LayerNorm)
